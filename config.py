...
SCALING_FEASIBILITY = torch.Tensor([0.4]).to(DEVICE)
SCALING_RECOVERY = torch.Tensor([0.07]).to(DEVICE)
SCALING_PURITY = torch.Tensor([0.2]).to(DEVICE)
SCALING_COST = torch.Tensor([0.16]).to(DEVICE)